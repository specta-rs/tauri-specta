...
use specta::{
    Format, Type, Types,
    datatype::{
        DataType, Field, Fields, Function, NamedDataType, NamedFields, NamedReferenceType,
        Primitive, Reference, Struct, StructBuilder,
    },
};
...
fn runtime(
    ...
    if enabled_commands {
        ...
        for command in &all_commands {
            ...
            let arguments =
                collect_arguments(command, &exporter, cfg, semantic_types_runtime_types)?;
    ...
    if let Some(_framework) = &cfg.tanstack {
        ...
        if has_queries {
            ...
            for command in &cfg.queries {
                ...
                field.deprecated = command.deprecated.clone();
                field.docs = build_command_docs(command, &arguments, jsdoc);
    ...
    // Runtime
    if has_typed_error
        || has_unwrap_typed_error
        || has_query_key_args
        || enabled_events
        || is_channel_transform_used
    {
...
fn command_uses_typed_error(command: &Function, types: &Types, cfg: &BuilderConfiguration) -> bool {
    cfg.error_handling == ErrorHandlingMode::Result
        && command
            .result()
    ...
    match command.result() {
        Some(dt) => {
            if let Some((ok, err)) = extract_std_result(dt, exporter.types) {
                let data = render_reference_dt_for_phase(
                    ok,
                    Phase::Serialize,
                    Phase::Deserialize,
                    exporter,
                ...
                let error = render_reference_dt_for_phase(
                    err,
                    Phase::Serialize,
                    Phase::Deserialize,
                    exporter,
