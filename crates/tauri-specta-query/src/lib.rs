...
//! ```
//!
//! Command names use `lowerCamelCase` by default and can be changed with
//! [`CommandSet::function_casing`]. Mutation arguments are passed to `mutate` as
//! one object; a mutation command with no arguments instead receives no input.
//!
//! Function casing changes the generated query, mutation, and command accessor
//! properties only. It does not change the Tauri command string or command
//! arguments in the runtime IPC message. Argument renaming is configured per
//! command; keep `#[tauri::command(rename_all = "...")]` and
//! `#[specta(rename_all = "...")]` aligned when using it.
//!
//! # Frameworks
//!
...
impl<R: Runtime> CommandSet<R> {
    ...
    pub fn new(queries: Commands<R>, mutations: Commands<R>) -> Self {
        ...
        Self {
            ...
            function_casing: Default::default(),
        }
    }
    ...
    /// Sets the casing used for generated query, mutation, and command accessors.
    ///
    /// This setting is forwarded to the returned [`tauri_specta::Builder`], so
    /// TanStack Query helpers always reference the command names generated by
    /// Tauri Specta. The default is [`Casing::CamelCase`].
    ///
    /// This changes frontend accessor properties only. It does not change the
    /// underlying Tauri command string or rename command arguments. Argument
    /// renaming is a separate per-command IPC contract configured with matching
    /// `#[tauri::command(rename_all = "...")]` and
    /// `#[specta(rename_all = "...")]` attributes.
    ///
    /// ```rust
    ...
    /// function casing configured on `self` is retained for the merged set.
    #[must_use]
    pub fn merge(&self, other: &Self) -> Self {
        ...
        Self {
            ...
            constants,
            function_casing: self.function_casing,
        }
    }
        ...
        let output = {
            ...
            if !self.queries.is_empty() {
                ...
                for function in &self.queries {
                    let name = self.function_casing.apply(&function.name).into_owned();
                    let name_json =
                        serde_json::to_string(&name).expect("failed to serialize query name");
                    let property = render_property_name(&name);
                    let command = render_member_access("commands", &name);

                    let options = format!(
                        "{{ queryKey: [{name_json}, ...args], queryFn: () => {command}(...args) }}"
                    );

                    output.push_str(&format!(
                        "\n\t{property}: (...args: Parameters<typeof {command}>) => {},",
                ...
                for function in &self.queries {
                    ...
                    output.push_str(&format!(
                        "\n\t{property}: (...args: Partial<Parameters<typeof {command}>>) => [{name_json}, ...args],"
                    ));
                }
                output.push_str("\n};");
            }
                ...
                for function in &self.mutations {
                    ...
                    let args = function
                        ...
                        .map(|(idx, (arg, _))| {
                            ...
                            format!("{arg}: Parameters<typeof {command}>[{idx}]")
                        })
                        .collect::<Vec<_>>();
...
mod tests {
    ...
    fn export(casing: Casing) -> String {
        let (query_bindings, builder) = CommandSet::<MockRuntime>::new(
            collect_commands![get_user],
            collect_commands![rename_user],
        )
        .function_casing(casing)
    ...
    #[test]
    fn function_casing_configures_helpers_and_commands() {
        let output = export(Casing::SnakeCase);

        assert!(output.contains("get_user: (...args: Parameters<typeof commands.get_user>)"));
        assert!(output.contains("queryFn: () => commands.get_user(...args)"));
        assert!(output.contains("rename_user: () => mutationOptions"));
        assert!(output.contains("commands.rename_user(input.userId, input.displayName)"));
        assert!(output.contains("get_user: (userId: number)"));
        assert!(output.contains("rename_user: (userId: number, displayName: string)"));
    }

    #[test]
    fn kebab_case_function_casing_emits_valid_property_access() {
        let output = export(Casing::KebabCase);

        assert!(
            output.contains(r#""get-user": (...args: Parameters<typeof commands["get-user"]>)"#)
        );
        assert!(output.contains(r#"queryFn: () => commands["get-user"](...args)"#));
        assert!(output.contains(r#""rename-user": () => mutationOptions"#));
        assert!(output.contains(r#"commands["rename-user"](input.userId, input.displayName)"#));
    }
}
